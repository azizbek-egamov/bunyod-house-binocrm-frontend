...
const GoogleSheetsConfigCreate = () => {
  ...
  const [tabMappings, setTabMappings] = useState({}); // { tabName: { phone_col, name_col, note_col, source_col, operator, is_active, columns, loadingColumns } }
  ...
  const handleInspect = async () => {
    if (!sheetUrl) {
      toast.error("Iltimos, Google Sheet URL manzilini kiriting.");
      return;
    }
    setInspecting(true);
    try {
      const res = await googleSheetsService.inspectConfig(sheetUrl);
      if (res.success) {
        setAvailableTabs(res.tabs || []);
        toast.success("Jadval varaqlari muvaffaqiyatli yuklandi. Kerakli varaqlarni tanlang.");

        setCheckedTabs([]);
        setTabMappings({});
        setActiveConfigTab(null);

        const firstTab = res.tabs?.[0];
        if (firstTab) {
          setCheckedTabs([firstTab]);
          setActiveConfigTab(firstTab);
          setTabMappings({
            [firstTab]: {
              phone_col: "",
              name_col: "",
      ...
      toast.error(
        error.response?.data?.message ||
          "Google Sheet-ni o'qishda xatolik yuz berdi. google_credentials.json faylini va ulashish huquqini tekshiring."
      );
    } finally {
      setInspecting(false);
    }
  ...
  const fetchColumnsForTab = async (tabName) => {
    setTabMappings((prev) => {
      if (!prev[tabName]) return prev;
      return {
        ...prev,
        [tabName]: { ...prev[tabName], loadingColumns: true },
      };
    });
    try {
      const res = await googleSheetsService.inspectConfig(sheetUrl, tabName);
      if (res.success) {
        setTabMappings((prev) => {
          if (!prev[tabName]) return prev;
          return {
      ...
      setTabMappings((prev) => {
        ...
        return {
          ...prev,
          [tabName]: { ...prev[tabName], loadingColumns: false },
        };
      });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const promises = checkedTabs.map((tabName) => {
    ...
    <div className="google-sheets-page google-sheets-form-page">
      <div className="page-header animate-fadeInDown">
        <div className="header-left">
          <h1 className="page-title">Yangi Google Sheets Qo'shish</h1>
          <p className="page-subtitle">
            Avtomatik tarzda Lead sifatida tortish uchun yangi jadval sozlamalari
          </p>
        </div>
            ...
            <div className="google-sheets-modal-form-full form-group">
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button
                  ...
                  disabled={inspecting || !sheetUrl}
                  className="btn-primary"
                  style={{ whiteSpace: "nowrap", padding: "0 16px", animation: "none", boxShadow: "none" }}
                >
                  {inspecting ? "Yuklanmoqda..." : "🔍 Tekshirish va yuklash"}
                </button>
              </div>
            </div>
...
export default GoogleSheetsConfigCreate;
